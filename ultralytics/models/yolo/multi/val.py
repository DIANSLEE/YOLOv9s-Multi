...
class MultiTaskValidator(DetectionValidator):
    ...
    def get_desc(self):
        ...
        return ('%22s' + '%11s' * 14) % ('Class', 'Images', 'Instances', 'Box(P', 'R', 'mAP50', 'mAP50-95)', 'Pose(P',
                                         'R', 'mAP50', 'mAP50-95)', 'Mask(P', 'R', 'mAP50', 'mAP50-95)')  # 改动点未改动，为啥只有一个class呢
    ...
    def init_metrics(self, model):
        ...
        self.stats = dict(tp_b=[], tp_p=[], tp_m=[], conf=[], pred_cls=[], target_cls=[], target_img=[])
    def _prepare_pred_mask(self, pred, pbatch, proto):
        """Prepares a batch for training or inference by processing images and targets."""
        predn = super()._prepare_pred(pred, pbatch)
        pred_masks = self.process(proto, pred[:, 6:], pred[:, :4], shape=pbatch["imgsz"])
        return predn, pred_masks
    def _prepare_pred_pose(self, pred, pbatch):
        ...
        for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
            ...
            if npr == 0:
                if nl:
                    ...
                    self.stats['tp_b'].append(correct_bboxes)
                    self.stats['tp_p'].append(correct_kpts)
                    self.stats['tp_m'].append(correct_masks)
                    self.stats['conf'].append(torch.zeros((0,), device=self.device))
                    self.stats['pred_cls'].append(torch.zeros((0,), device=self.device))
                    self.stats['target_cls'].append(cls.squeeze(-1))
                    self.stats['target_img'] = [cls.unique()]
                    if self.args.plots:
                        self.confusion_matrix.process_batch(detections=None, gt_bboxes=bbox, labels=cls.squeeze(-1))
                continue
            ...
            if nl:
                ...
                if self.args.plots:
                    ...
                    self.confusion_matrix.process_batch(predn, bbox, cls)  # 改动点
            ...
            self.stats['tp_p'].append(correct_kpts)
            self.stats['target_cls'].append(cls.squeeze(-1))
            self.stats['conf'].append(pred[:,4])
            self.stats['pred_cls'].append(pred[:,5])
            self.stats['target_img'].append(cls.unique())
